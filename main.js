...
function initCode()
{
    ...
    var newUserRef1 = firebase.database().ref("openedweek11/" + user["eml"]);
    newUserRef1.update ({
        "name": user["eml"],
        });
    ...
    firebase.database().ref('week11answers').on('value', function(snap){
...
function endGame()
{
    var newUserRef = firebase.database().ref("week11scores/" + user["eml"]);
