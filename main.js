...
function initCode()
{
    ...
    var newUserRef1 = firebase.database().ref("openedweek12/" + user["eml"]);
    newUserRef1.update ({
        "name": user["eml"],
        });
    ...
    firebase.database().ref('week12answers').on('value', function(snap){
...
function endGame()
{
    var newUserRef = firebase.database().ref("week12scores/" + user["eml"]);
