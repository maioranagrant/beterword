...
function initCode()
{
    ...
    var newUserRef1 = firebase.database().ref("openedweek10/" + user["eml"]);
    newUserRef1.update ({
        "name": user["eml"],
        });
    ...
    firebase.database().ref('week10answers').on('value', function(snap){
...
function endGame()
{
    var newUserRef = firebase.database().ref("week10scores/" + user["eml"]);
