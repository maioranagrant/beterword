...
function initCode()
{
    ...
    var newUserRef1 = firebase.database().ref("openedweek3/" + user["eml"]);
    newUserRef1.update ({
        "name": user["eml"],
        });
    ...
    firebase.database().ref('week3answers').on('value', function(snap){
...
function endGame()
{
    var newUserRef = firebase.database().ref("week3scores/" + user["eml"]);
