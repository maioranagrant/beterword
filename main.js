...
function initCode()
{
    ...
    var newUserRef1 = firebase.database().ref("openedweek16/" + user["eml"]);
    newUserRef1.update ({
        "name": user["eml"],
        });
    ...
    firebase.database().ref('week16answers').on('value', function(snap){
...
function endGame()
{
    var newUserRef = firebase.database().ref("week16scores/" + user["eml"]);
