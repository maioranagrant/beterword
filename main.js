...
function initCode()
{
    ...
    var newUserRef1 = firebase.database().ref("openedweek4/" + user["eml"]);
    newUserRef1.update ({
        "name": user["eml"],
        });
    ...
    firebase.database().ref('week4answers').on('value', function(snap){
...
function endGame()
{
    var newUserRef = firebase.database().ref("week4scores/" + user["eml"]);
