...
function initCode()
{
    ...
    var newUserRef1 = firebase.database().ref("openedweek9/" + user["eml"]);
    newUserRef1.update ({
        "name": user["eml"],
        });
    ...
    firebase.database().ref('week9answers').on('value', function(snap){
...
function endGame()
{
    var newUserRef = firebase.database().ref("week9scores/" + user["eml"]);
