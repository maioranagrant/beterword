...
function initCode()
{
    ...
    var newUserRef1 = firebase.database().ref("openedweek8/" + user["eml"]);
    newUserRef1.update ({
        "name": user["eml"],
        });
    ...
    firebase.database().ref('week8answers').on('value', function(snap){
...
function endGame()
{
    var newUserRef = firebase.database().ref("week8scores/" + user["eml"]);
